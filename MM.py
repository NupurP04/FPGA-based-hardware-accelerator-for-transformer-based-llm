...
def out_feature_wait(timeout_sec=5.0):
    start = time.time()
    while not (out_f_dma.read(0x34) & XAXIDMA_IDLE_MASK):
        if time.time() - start > timeout_sec:
            raise TimeoutError("Timeout waiting for output DMA to go idle.")
...
def in_feature_wait(timeout_sec=5.0):
    start = time.time()
    while not (in_f_dma.read(0x4) & XAXIDMA_IDLE_MASK):
        if time.time() - start > timeout_sec:
            raise TimeoutError("Timeout waiting for input feature DMA.")
...
def in_weight_wait(timeout_sec=5.0):
    start = time.time()
    while not (in_w_dma.read(0x4) & XAXIDMA_IDLE_MASK):
        if time.time() - start > timeout_sec:
            raise TimeoutError("Timeout waiting for weight DMA.")
...
def mat_mul(A, B, C,shift=0):
    ...
    print("Starting output DMA (S2MM)...")
    out_feature_transfer(C)

    print("Starting input feature DMA (MM2S)...")
    in_feature_transfer(A)

    print("Starting input weight DMA (MM2S)...")
    in_weight_transfer(B)

    print("Waiting for output DMA to complete...")
    out_feature_wait()
    print("Hardware mat_mul complete.")
